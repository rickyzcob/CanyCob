...
{{--                            <div class="text-sm leading-4 font-normal"> Acordo gerado em  : {{ formatDate($itemAgreement['created_at'] ) }}</div>--}}
                            <div>{{ $itemAgreement['type'] }} </div>
                            <div>No valor de {{ formatMoney($itemAgreement['agreements_amount']) }} </div>
                            <div>em {{ $itemAgreement['installments'] }} x de {{ formatMoney($itemAgreement['installment_value']) }} </div>
                        </div>
                        <div>
