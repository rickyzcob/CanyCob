    ...
    <div class="container xl justify-center items-center min-h-[640px]">
        <nav class="py-5">
            <ol class="list-reset flex">
                ...
                <li class="text-gray-100">Cadastro</li>
            </ol>
        </nav>
        <div class="card p-5 gap-4 p-2">
            <div class="flex items-start justify-between  border-b-2 mb-2 ">
                <h1 class="text-lg text-gray-600 font-semibold p-2">Franqueados</h1>
                <x-button icon="home" positive label="Cadastrar" x-data={}
                          x-on:click="livewire.emitTo('components.open-modal', 'showModal', 'franchising.form', {'id' : null})">
                </x-button>
            </div>

            @livewire('franchising.filter')

            <div class="justify-center items-center mb-2">
                <p> Lista de Franqueados Cadastrados</p>
            </div>

            @livewire('franchising.table')
        </div>
    </div>
