<div>
    <x-card cardClasses="h-[462px] border-l-4 border-red-600">
        <div class="flex items-start justify-between border-b-2 mb-2">
            <h1 class="text-base text-gray-600 font-semibold py-2">Unidades em Cobrança</h1>
        </div>
                                ...
                                @if($itemCharge['agreement'] == 0)
                                    <div class="flex gap-2">
                                        @can('add_historic_charges')
                                            <x-button wire:click="openModal('charges.historic.form', {'id': {{$itemCharge['id']}} })" positive sm icon="annotation" label="Cobrar"/>
                                        @endcan
                                        @can('whatsapp_proposal_charges')
                                            <x-button class="fa fa-whatsapp" wire:click="openModal('charges.whatsapp.form', {'id': {{$itemCharge['id']}} })" teal sm label="Whatsapp"/>
                                        @endcan
                                        @can('simulation_charges')
                                            @if($itemCharge['total_amount_corrected'] > $response->configuration['value_agreement'])
                                                <x-button wire:click="openModal('charges.simulation.form', {'id': {{$itemCharge['id']}} })" blue sm icon="refresh" label="Simular"/>
                                            @endif
                                        @endcan
                                    </div>
                                @endif
